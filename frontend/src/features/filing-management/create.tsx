...
type Step = 1 | 2 | 3 | 4 | 5
type SubPortRuleMode = 'random' | 'sequential' | 'fixed_suffix'
type AllocationMode = SubPortRuleMode | 'qualification_only'
...
export function FilingCreatePage() {
  ...
  const [allocationMode, setAllocationMode] = useState<AllocationMode>('random')
  ...
  const { data: qualData } = useQuery({
    queryKey: [
      'qualifications',
      { page: qualPage, page_size: 10, sms_signature: qualSearch || undefined },
    ],
    queryFn: () =>
      getQualifications({
        page: qualPage,
        page_size: 10,
        sms_signature: qualSearch || undefined,
      }),
  })

  // Fetch export groups
  const {
    data: exportGroupsData,
    isLoading: exportGroupsLoading,
    isError: exportGroupsError,
  } = useQuery({
  ...
  const allPorts: PortInfo[] = useMemo(
    () => portData?.data ?? [],
    [portData?.data]
  )
  ...
  const selectedExportGroup = exportGroupsData?.data?.find(
    (g: ExportGroup) => g.id === exportGroupId
  )
  ...
  const filteredGroups = useMemo(() => {
    ...
    for (const [key, ports] of Object.entries(portGroups)) {
      if (
        portCarrierFilter !== '__all__' &&
        !ports.some((p) => p.carrier === portCarrierFilter)
      )
        continue
      if (search) {
  ...
  const selectedPortIdList = useMemo(
    () =>
      Object.entries(selectedPortIds)
        .filter(([, v]) => v)
        .map(([id]) => id),
    [selectedPortIds]
  )
  ...
  const estimatedRows =
    allocationMode === 'qualification_only'
      ? selectedIds.length
      : selectedIds.length * selectedPortIdList.length
  ...
  const handleSelectRule = (id: string) => {
    ...
    if (!rule) return
    setAllocationMode(rule.mode as SubPortRuleMode)
    if (rule.mode === 'fixed_suffix') {
      ...
      if (start !== undefined && start !== null)
        setSubPortRangeStart(String(start))
  ...
  const handleSaveRule = () => {
    ...
    if (allocationMode === 'qualification_only') return
    saveRuleMutation.mutate({
      ...
      config:
        allocationMode === 'fixed_suffix'
          ...
          : {
              range_start: Number(subPortRangeStart),
              range_end: Number(subPortRangeEnd),
            },
    ...
    createMutation.mutate(
      {
        ...
        port_ids:
          allocationMode === 'qualification_only' ? [] : selectedPortIdList,
        export_group_id: exportGroupId,
        group_by_field:
          groupByField === '__none__' ? undefined : groupByField || undefined,
        task_name: taskName.trim() || undefined,
        auto_allocate_sub_ports: allocationMode !== 'qualification_only',
        sub_port_range_start:
          allocationMode === 'qualification_only'
            ? undefined
            : Number(subPortRangeStart),
        sub_port_range_end:
          allocationMode === 'qualification_only'
            ? undefined
            : Number(subPortRangeEnd),
        allocation_mode: allocationMode,
        fixed_suffix:
          allocationMode === 'fixed_suffix'
            ? fixedSuffix.trim() || undefined
            : undefined,
      },
      {
        onSuccess: (task) => {
    ...
    <>
      ...
      <Main>
        <div className='mb-2 flex flex-wrap items-center justify-between space-y-2'>
          <div className='flex items-center gap-4'>
            <Button
              variant='ghost'
              size='icon'
              onClick={() => navigate({ to: '/filing-management' })}
            >
        ...
        <div className='space-y-6'>
          ...
          <div className='flex items-center gap-2'>
            {([1, 2, 3, 4, 5] as Step[]).map((s, i) => (
              <div key={s} className='flex items-center gap-2'>
                ...
                {i < 4 && <div className='bg-border mx-2 h-px w-8' />}
              </div>
            ))}
          </div>
            ...
            <Card>
              ...
              <CardContent className='space-y-4'>
                <div className='flex flex-wrap items-center gap-3'>
                  ...
                  />
                  <Select
                    value={portCarrierFilter}
                    onValueChange={setPortCarrierFilter}
                  >
                  ...
                  <span className='text-muted-foreground text-sm'>
                    已选 {selectedGroupCount} 个主端口 /{' '}
                    {selectedPortIdList.length} 行，可不选端口后选择仅导出资质
                  </span>
                </div>
          ...
          {/* Step 3: Configure generation mode */}
          {step === 3 && (
            <Card>
              <CardHeader>
                <CardTitle>配置生成模式</CardTitle>
              </CardHeader>
              <CardContent className='space-y-4'>
                <div className='flex items-end gap-3'>
                  <div className='flex flex-col gap-1'>
                    ...
                    <Select
                      ...
                      onValueChange={handleSelectRule}
                    >
                  ...
                  <Button
                    ...
                    disabled={allocationMode === 'qualification_only'}
                  >
                    保存为规则
                  </Button>
                </div>
                ...
                <div className='space-y-2'>
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value='qualification_only'>
                        无需子端口，仅导出资质
                      </SelectItem>
                    </SelectContent>
                  </Select>
                ...
                <div className='bg-muted/50 rounded p-3 text-sm'>
                  {allocationMode === 'qualification_only'
                    ? `预计导出 ${estimatedRows} 行资质`
                    : `预计生成 ${selectedIds.length * selectedGroupCount} 个子端口（资质 ${selectedIds.length} × 主端口 ${selectedGroupCount}）`}
                </div>

                {/* 子端口号预览 */}
                {allocationMode !== 'qualification_only' && (
                  <div className='mt-4 rounded border p-3 text-sm'>
                    <h4 className='mb-2 font-medium'>子端口号预览</h4>
                    {selectedPortIdList.map((id) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span>
                            子端口扩展码: {p.sub_port_number || '(自动生成)'}
                          </span>
                          <span>
                            短信子端口号: {p.main_port_number}
                            {p.sub_port_number || 'XXXXXX'}
                          </span>
                        </div>
                      )
                    })}
              ...
              <CardContent className='space-y-6'>
                <div className='bg-muted/50 grid grid-cols-2 gap-4 rounded-lg p-4'>
                  ...
                  <div>
                    ...
                    <p className='text-lg font-medium'>
                      {groupByField && groupByField !== '__none__'
                        ? getFieldLabel(groupByField)
                        : '无'}
                    </p>
                  </div>
                  <div>
...
function RangeAvailability({
  ...
  const { data } = useQuery({
    ...
    queryFn: () =>
      getSubPortAvailability(mainPortNumbers, rangeStart, rangeEnd),
    enabled:
      mainPortNumbers.length > 0 && rangeStart > 0 && rangeEnd > rangeStart,
  })
