...
export function LoginLogsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'success':
        return <StatusTag status='成功' />
      case 'failed':
        return <StatusTag status='失败' />
      default:
        return <StatusTag status='未知' />
    }
  }
      ...
      <Main>
        <div className='mb-2 flex flex-wrap items-center justify-between space-y-2'>
          <div>
            ...
            <p className='text-muted-foreground'>查看系统登录记录和安全信息</p>
          </div>
        ...
        {logs.length === 0 && !isLoading && (
          <EmptyState
            title='暂无登录日志'
            description='用户登录后，这里会展示登录结果、IP 地址和设备信息。'
          />
        )}
      </Main>
