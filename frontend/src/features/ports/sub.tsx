...
import { getSubPorts, createSubPort, updateSubPort, deleteSubPort } from '@/lib/api/ports'
...
export function SubPortsPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setDeleteDialogOpen(false); setToDelete(undefined)
    },
  ...
  const columns = useMemo<ColumnDef<SubPort>[]>(() => [
    ...
    { accessorKey: 'main_port_number', header: '所属主端口', cell: ({ getValue }) => getValue() || '-' },
    {
      accessorKey: 'carrier',
      header: '运营商',
    ...
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        return <Badge variant={s === '已分配' ? 'default' : s === '停用' ? 'destructive' : 'secondary'}>{s}</Badge>
      },
    },
    {
      id: 'actions',
  ...
  return (
    <>
      ...
      <Main>
        <div className='mb-2 flex flex-wrap items-center justify-between space-y-2'>
          <div>
            ...
            <p className='text-muted-foreground'>管理子端口号、主端口归属及分配状态</p>
          </div>
          <div className="flex space-x-2">
            <Button onClick={() => { setSelected(undefined); setDialogOpen(true) }}>
              <Plus className="mr-2 h-4 w-4" />新建子端口
            </Button>
            <Button variant="outline" onClick={() => queryClient.invalidateQueries({ queryKey: ['sub-ports'] })} disabled={isLoading}>
      ...
      <SubPortDialog open={dialogOpen} onOpenChange={setDialogOpen} port={selected} onSubmit={selected ? handleUpdate : handleCreate} />
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>确认删除</AlertDialogTitle>
            <AlertDialogDescription>确定要删除子端口 "{toDelete?.port_number}" 吗？</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
